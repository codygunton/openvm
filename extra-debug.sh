...
if [ $BTESTS = "1" ]; then
    ...
    ./run test --extra openvm --build-only
else
...
if [ $RUN = "1" ]; then
    echo "🐛 Running extra test: $PATTERN"

    # Run test and capture exit code, showing only last 20 lines
    set +e # Temporarily disable exit on error
    ./run debug --extra openvm "$PATTERN" 2>&1 | tail -20
    EXIT_CODE=${PIPESTATUS[0]}
    set -e # Re-enable exit on error

    # Find the test directory and files
    # First try exact match (e.g., fadd_b3 matches fadd_b3-01.S but not fadd_b30-01.S)
    ...
    if [ $SIGS = "1" ] && [ -n "$TEST_DIR" ] && [ -d "$TEST_DIR" ]; then
        ...
        # For extra tests, use reference signature from ref-sigs-extra directory
        # TEST_DIR is like: test-results/openvm/rv32i_m/F/src/fnmadd-baby.S/dut
        # Extract path after rv32i_m: F/src/fnmadd-baby.S
        TEST_PATH=$(echo "$TEST_DIR" | sed 's|test-results/openvm/rv32i_m/||' | sed 's|/dut$||')
        # Go up to repo root and use ref-sigs-extra
        cd ..
        REF_SIG="ref-sigs-extra/${TEST_PATH}/ref/Reference-sail_c_simulator.signature"
        ...
        if [ ! -f "zkevm-test-monitor/$DUT_SIG" ]; then
            ...
            echo ""
            echo "   💡 To create reference signatures, run:"
            echo "      ./generate_ref_sigs_extra.sh"
            cd zkevm-test-monitor
        else
