...
```bash
cargo run --release --bin cargo-openvm -- openvm run --exe examples/floats/build/test.elf --config examples/floats/openvm.toml
```

**Option 2:** From the examples/floats directory (after building cargo-openvm):
```bash
# First, build cargo-openvm if you haven't already
cd /path/to/openvm
cargo build --release --bin cargo-openvm

# Then run from examples/floats directory
cd examples/floats
../../target/release/cargo-openvm openvm run --exe build/test.elf --config openvm.toml
```

## What it tests

1. Initializes float library pointer at 0x00100000
