...
use elf::{
    abi::{EM_RISCV, ET_EXEC, PF_X, PT_LOAD, SHF_EXECINSTR},
    endian::LittleEndian,
    file::Class,
    ElfBytes,
};
...
impl Elf {
    ...
    pub fn decode(input: &[u8], max_mem: u32) -> eyre::Result<Self> {
        ...
        // Get sections to find executable code and track their addresses
        let sections = elf.section_headers().ok_or_else(|| eyre::eyre!("Missing section headers"))?;

        // Collect executable sections with their addresses
        let mut exec_sections: Vec<(u32, u32, u32)> = Vec::new(); // (vaddr, size, offset)
        for section in sections.iter() {
            if (section.sh_flags & SHF_EXECINSTR as u64) != 0 {
                let vaddr: u32 = section.sh_addr.try_into()?;
                let size: u32 = section.sh_size.try_into()?;
                let offset: u32 = section.sh_offset.try_into()?;

                if vaddr < base_address {
                    base_address = vaddr;
                }

                exec_sections.push((vaddr, size, offset));
            }
        }

        // Sort sections by virtual address
        exec_sections.sort_by_key(|(vaddr, _, _)| *vaddr);

        // Extract instructions, filling gaps with 0 (which will transpile to None)
        let mut current_addr = base_address;

        for (vaddr, size, offset) in exec_sections {
            eprintln!("ELF: Extracting section at vaddr=0x{:08x}, size=0x{:x}", vaddr, size);

            // Fill gap before this section with zeros (padding)
            while current_addr < vaddr {
                instructions.push(0);  // Will become None during transpilation
                current_addr += WORD_SIZE as u32;
            }

            // Extract instructions from this section
            for i in (0..size).step_by(WORD_SIZE) {
                let addr = vaddr + i;
                if addr >= max_mem {
                    bail!("address [0x{addr:08x}] exceeds maximum");
                }

                let mut word = 0;
                let len = min(size - i, WORD_SIZE as u32);
                for j in 0..len {
                    let file_offset = (offset + i + j) as usize;
                    let byte = input.get(file_offset).context("Invalid section offset")?;
                    word |= u32::from(*byte) << (j * 8);
                }
                instructions.push(word);
                current_addr = addr + WORD_SIZE as u32;
            }
        }

        // Second pass: load all PT_LOAD segments into memory image
        for segment in segments.iter().filter(|x| x.p_type == PT_LOAD) {
            ...
            // Load segment into memory image (data + code)
        ...
        // Extract symbol table AFTER we know base_address, so we can convert RISC-V addresses to PC indices
        #[cfg(feature = "function-span")]
        {
            if let Some((symtab, stringtab)) = elf.symbol_table()? {
                // First pass: extract function names and create buffer for guest symbols file
                ...
                for symbol in symtab.iter() {
                    if symbol.st_symtype() == elf::abi::STT_FUNC {
                        ...
                        // Store the actual demangled name for use in fn_bounds
                        fn_names.insert(symbol.st_name, demangled_name.clone());

                        // Create buffer entry for guest symbols file
                        if let Entry::Vacant(e) = offsets.entry(symbol.st_name) {
                            let offset = buf.len();
                            e.insert(offset);
                            buf.extend_from_slice(demangled_name.as_bytes());
                            buf.push(0);
                        }
                    }
                }

                // Second pass: create fn_bounds with PC indices and actual names
                for symbol in symtab.iter() {
                    if symbol.st_symtype() == elf::abi::STT_FUNC {
                        let riscv_addr = symbol.st_value as u32;
                        let riscv_end = (symbol.st_value + symbol.st_size) as u32;

                        // Convert RISC-V addresses to OpenVM PC indices
                        let pc_start = (riscv_addr - base_address) / (WORD_SIZE as u32);
                        let pc_end = if riscv_end > riscv_addr {
                            (riscv_end - base_address) / (WORD_SIZE as u32)
                        } else {
                            pc_start
                        };

                        let name = fn_names.get(&symbol.st_name)
                            .expect("Function name should exist from first pass")
                            .clone();
                ...
                eprintln!("✓ Extracted {} function symbols from ELF", fn_bounds.len());
            } else {
                eprintln!("⚠ No symbol table found in ELF");
            }
        }
