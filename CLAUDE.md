...
We have the RISC-V architecture tests set up to run in zkevm-test-monitor, which is a symlink to an external repo. Rather than interact with these directly,
you will use the two scripts elf_build_run.sh and, only once the toy example has been validated to work correctly, riscof_build_run.sh.
